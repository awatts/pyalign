...
def get_wave_length(wave_fn):
    ...
    """
    wf = wave.open(wave_fn, 'r') # does 'wave' implement a context manager?
...
def clean_transcript(trans_fn):
    ...
    with open(trans_fn, 'r') as transcript:
        ...
        for line in transcript:
            #FIXME: what are we getting rid of and why?
            line = re.sub('@\S+', '', line)
            line = re.sub('\/', '', line)
...
def align(audio_fn, trans_fn):
    """
    Force align the audio to the transcript and output the phonetic and word
    level segmentations, as well as an 'out of dictionary' file
    """
    ...
    return subprocess.call(['sphinx3_align',
                      '-agc', 'none',
                      '-ctl', 'ctl',
                      '-cepext', 'mfc',
                      '-dict', 'vocab.dic',
